...
Film suchen: <input type="text" name="recherche" class="form">
<select name="database">
	<option value="TMDb">TMDb</option>
	<option value="Allocine">Filmstarts</option>
</select>
			<input type="submit" value="Suchen" class="form">
			</form></p>
		</div>
		<div id="tabs-3">
			...
			else echo 'Kein Poster vorhanden<br>';
...
switch($_GET['action']){
	...
	?>
	<form method="POST" action="update.php?link=<?php echo urlencode($_GET['link']); ?>&oldcode=<?php echo $_GET['oldcode'];?>&action=auto" class="nyroModal" style="text-align:center;">Neue Suche : <input type="text" name="recherche" class="form">
<select name="database">
	<option value="Allocine">Filmstarts</option>
	<option value="TMDb">TMDb</option>
</select>
	...
	for($i=0;$i<count($recherche);$i++){
		...
		echo '<br><a href="update.php?link='.urlencode($_GET['link']).'&code='.$recherche[$i]['code'].'&oldcode='.$_GET['oldcode'].'&database='.$_POST['database'].$img_link.'&action=autoupdate" class="nyroModal"><input type="button" value="Auswählen"></a>';
		echo '</li>';
	}
	echo '</ul>';
	break;
	...
	echo '<img src="images/check.png" alt="ok">'.$_GET['link'].' geupdated!<br />Seite neu laden um die Änderungen wirksam zu machen.</div>';
	break;
